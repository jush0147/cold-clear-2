...
enum Kind { Legacy, S2 }
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum Outcome { P0, P1, Draw }
#[derive(Default)]
struct Score { s2_wins: u64, legacy_wins: u64, draws: u64 }
struct Player {
    bot: Bot,
...
fn main() {
    ...
    for seed in 0..seeds {
        record(&mut score, duel(seed, Kind::Legacy, Kind::S2, max_pieces, nodes, garbage_cap, charge_value, surge_value, shape_value), Kind::Legacy, Kind::S2);
        record(&mut score, duel(seed, Kind::S2, Kind::Legacy, max_pieces, nodes, garbage_cap, charge_value, surge_value, shape_value), Kind::S2, Kind::Legacy);
    }
...
fn record(score: &mut Score, outcome: Outcome, p0: Kind, p1: Kind) {
    match outcome {
        ...
        Outcome::P0 => match p0 { Kind::S2 => score.s2_wins += 1, Kind::Legacy => score.legacy_wins += 1 },
        Outcome::P1 => match p1 { Kind::S2 => score.s2_wins += 1, Kind::Legacy => score.legacy_wins += 1 },
    }
}
...
fn duel(seed: u64, p0_kind: Kind, p1_kind: Kind, max_pieces: u64, nodes: u64, garbage_cap: usize, charge_value: f32, surge_value: f32, shape_value: f32) -> Outcome {
    ...
    for round in 0..max_pieces {
        for active in 0..2 {
            let other = 1 - active;
            if play_turn(&mut players, active, other, seed, round, nodes, garbage_cap, &sequence) {
...
fn play_turn(players: &mut [Player; 2], active: usize, other: usize, seed: u64, round: u64, node_budget: u64, garbage_cap: usize, sequence: &[Piece]) -> bool {
    let mut searched = 0;
    ...
    let Some(&mv) = players[active].bot.suggest().first() else { return true; };
    ...
    for i in 0..outgoing {
        players[other].incoming.push_back(garbage_hole(seed, active as u64, round, i as u64));
    }

    let rise = garbage_cap.min(players[active].incoming.len());
    for _ in 0..rise {
        let hole = players[active].incoming.pop_front().unwrap();
        if players[active].bot.add_garbage_line(hole) { return true; }
    }
...
fn garbage_hole(seed: u64, sender: u64, round: u64, line: u64) -> usize {
    let mut x = seed ^ sender.wrapping_mul(0x9E37_79B9_7F4A_7C15) ^ round.wrapping_mul(0xBF58_476D_1CE4_E5B9) ^ line.wrapping_mul(0x94D0_49BB_1331_11EB);
    x ^= x >> 30;
...
fn arg_u64(name: &str, default: u64) -> u64 {
    ...
    while let Some(arg) = args.next() {
        if arg == name { return args.next().and_then(|v| v.parse().ok()).unwrap_or(default); }
    }
    default
}
