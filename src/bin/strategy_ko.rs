//! KO-only A/B: legacy evaluation versus one pending-safety hypothesis.
...
#[derive(Clone, Copy)]
struct Settings { seeds: u64, start: u64, nodes: u64, candidate: f32 }
fn settings(args: &[String]) -> Result<Settings, String> {
    let mut s = Settings { seeds: 20, start: 100, nodes: 10000, candidate: 1.0 };
    ...
    for p in args.chunks_exact(2) {
        match p[0].as_str() {
            ...
            "--candidate" => s.candidate = p[1].parse().map_err(|_| "invalid candidate")?,
            _ => return Err(format!("unknown flag {}; no piece cap or tiebreak", p[0])),
        }
    }
    if s.seeds == 0 || s.nodes < 1000 || !s.candidate.is_finite() || s.candidate < 0.0 {
        return Err("positive seeds, >=1000 nodes, finite nonnegative candidate required".into());
    }
...
impl Sequence {
    ...
    fn get(&mut self, index: usize) -> Piece {
        while self.pieces.len() <= index {
            ...
            bag.shuffle(&mut self.rng); self.pieces.extend(bag);
        }
        self.pieces[index]
    }
}
...
struct Player {
    bot: Bot, incoming: VecDeque<Packet>, revealed: usize, pieces: u32, sent: u32,
    stats: Statistics, unused: u64, pressure_turns: u64,
}
fn make_player(visible: Vec<Piece>, safety: f32) -> Result<Player, String> {
    // Remainder deduced only from revealed pieces, never copied from hidden RNG.
    let mut bag = EnumSet::all();
    for &p in &visible { bag.remove(p); }
    let start = Start { board: Board::default(), queue: visible, hold: None, combo: 0,
        back_to_back: false, b2b_count: 0, randomizer: Randomizer::SevenBag { bag_state: bag } };
    let mut config = BotConfig::legacy();
    config.freestyle_weights.softdrop = 0.0;
    config.freestyle_weights.pending_safety = safety;
    Ok(Player { bot: try_create_bot(start, Arc::new(config))?, incoming: VecDeque::new(),
        revealed: PREVIEW, pieces: 0, sent: 0, stats: Statistics::default(), unused: 0, pressure_turns: 0 })
}
...
/// No opponent, sequence generator or authority hole RNG is accessible here.
fn choose(bot: &mut Bot, incoming: &[GarbagePacket], pieces: u32, sent: u32, budget: u64)
    -> Result<(Placement, Statistics), String> {
    let before = bot.state();
    let visible = bot.player_pieces();
    if visible.current.is_none() || visible.next.len() != 5 { return Err("observation must have current + NEXT x5".into()); }
    ...
    for scenario in 0..scenarios {
        ...
        let mut stalled = 0;
        with_search_seed(0xC01D_C1EAu64 ^ (pieces as u64).wrapping_mul(0x9E3779B97F4A7C15) ^ scenario, || {
        ...
        if here.is_empty() { return Err("search produced no move despite legal authority state".into()); }
        if scenario == 0 {
            ranked = here.iter().map(|&(m,v)| (m,v as f64)).collect();
        } else {
            if here.len() != ranked.len() { return Err("root action set changed before unknown holes revealed".into()); }
            for (mv, score) in &mut ranked {
                let value = here.iter().find(|(m,_)| m == mv).ok_or("missing common root action")?.1;
                *score += value as f64;
            }
        }
    }
    ...
    let mv = ranked[0].0;
    // Forecasts must never alter authority or be applied twice.
    bot.set_forecast(Forecast::default());
    if bot.state() != before || bot.player_pieces() != visible { return Err("search mutated authority state".into()); }
    Ok((mv,total))
}
...
fn consume(q: &mut VecDeque<Packet>, mut n: u32) {
    while n > 0 {
        ...
        let k = n.min(p.lines); n -= k; p.lines -= k;
...
fn duel(seed: u64, swapped: bool, s: Settings) -> Result<Value, String> {
    ...
    let safety = if swapped { [s.candidate,0.0] } else { [0.0,s.candidate] };
    let mut players = [make_player(visible.clone(),safety[0])?, make_player(visible,safety[1])?];
    let mut holes = [StdRng::seed_from_u64(seed ^ 0xA2E1_937F), StdRng::seed_from_u64(seed ^ 0x5BCA_903D)];
    let mut digest = 0xcbf29ce484222325u64;
    let started = Instant::now();
    let (loser, reason) = 'game: loop {
        for active in 0..2 {
            if !players[active].bot.has_legal_move() { break 'game (active, "no_legal_placement"); }
            ...
            for attack in tetrio::attack(&info).packets() {
                ...
                if sent != 0 { outgoing.push(Packet { lines: sent, hole: holes[active].gen_range(0..10) }); }
            }
            p.pieces = p.pieces.checked_add(1).ok_or("piece counter overflow")?;
            if info.lines_cleared == 0 {
                for _ in 0..8 {
                    let hole = match p.incoming.front() { Some(packet) => packet.hole, None => break };
                    consume(&mut p.incoming, 1);
                    if p.bot.add_garbage_line(hole) { break 'game (active,"garbage_overflow"); }
                }
            }
            let refill = p.bot.preview_refill_needed();
            if !(1..=2).contains(&refill) { return Err("invalid hold/preview consumption".into()); }
            for _ in 0..refill { p.bot.new_piece(sequence.get(p.revealed)); p.revealed += 1; }
    ...
    Ok(json!({"type":"game","seed":seed,"swapped":swapped,
        "winner":if winner==candidate_slot {"candidate"} else {"baseline"},
        "winner_slot":winner,"reason":reason,"pieces":[players[0].pieces,players[1].pieces],
        "node_budget":s.nodes,"candidate_safety":s.candidate,
...
fn main() -> Result<(), String> {
    ...
    println!("{}",json!({"type":"protocol","candidate":"legacy + pending_safety","coefficient":s.candidate,
        "baseline":"legacy weights; shared zero-gravity/no-softdrop-cost policy","speculate":true,
        "information":"own current/hold/next5/board/b2b/combo/observable incoming/history only",
        "arena":"existing turn-based S2: alternating locks, outgoing active on receipt, cap8 per nonclear, no clock",
        "unverified":"live travel and Surge activation timing; unknown clean-hole scenarios approximate",
        "seed_start":s.start,"seed_count":s.seeds,"nodes_per_move":s.nodes,
        "result_rule":"KO only; errors/timeouts uncompleted, never a win or draw"}));
    let (mut candidate,mut baseline) = (0,0);
    for seed in s.start..s.start+s.seeds {
        let mut control_trace: Option<Value> = None;
        for swapped in [false,true] {
            let game = duel(seed,swapped,s).map_err(|e|format!("ABORT seed={seed} swapped={swapped}: {e}; no winner assigned"))?;
            if s.candidate == 0.0 {
                if let Some(t) = &control_trace { if t != &game["trace_hash"] { return Err("A/A pair not repeatable".into()); } }
                control_trace = Some(game["trace_hash"].clone());
            }
            if game["winner"]=="candidate" { candidate+=1; } else { baseline+=1; }
            println!("{game}");
        }
    }
    println!("{}",json!({"type":"summary","completed_games":candidate+baseline,"candidate_wins":candidate,"baseline_wins":baseline,"all_pairs_complete":true}));
    Ok(())
}
#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn rejects_piece_caps_and_invalid_budgets() {
        for args in [vec!["--pieces","120"],vec!["--nodes","0"],vec!["--candidate","NaN"]] {
    ...
    fn five_next_does_not_limit_search_depth() {
        ...
        let p=make_player(visible,0.0).unwrap();
    ...
    fn hard_budget_and_authority_unchanged() {
        ...
        let visible=(0..6).map(|i|seq.get(i)).collect();
        let mut p=make_player(visible,0.0).unwrap();
