// Conditionally compile the new wasm_adapter module
...
// Keep native-only functionality separate
#[cfg(not(target_arch = "wasm32"))]
mod native {
    use crate::bot::{Bot, BotConfig, BotOptions};
    use crate::data::GameState;
    use crate::sync::BotSyncronizer;
    use crate::tbp::{self, BotMessage, FrontendMessage, Randomizer};
    use enumset::EnumSet;
    use futures::prelude::*;
            ...
            match msg {
                ...
                FrontendMessage::NewPiece { piece } => {
                    if let Some(mut start) = waiting_on_first_piece.take() {
                        if let tbp::Randomizer::SevenBag { bag_state } = &mut start.randomizer {
...
// Re-export native run function if not compiling for wasm
#[cfg(not(target_arch = "wasm32"))]
pub use native::run;
